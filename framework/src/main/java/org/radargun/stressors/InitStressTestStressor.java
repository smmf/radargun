...
public class InitStressTestStressor extends AbstractCacheWrapperStressor {

   private static final Log log = LogFactory.getLog(InitStressTestStressor.class);
...
//   private volatile InitStressorCompletion completion;
   private volatile boolean finished = false;
   private volatile boolean terminated = false;
   private AtomicLong keysLoaded = new AtomicLong(0);
   
   protected List<InitStressor> stressors = new ArrayList<InitStressor>(numThreads);
   ...
   public Map<String, Object> stress(CacheWrapper wrapper) {
      init(wrapper);
//      InitStressorCompletion completion;
//      if (durationMillis > 0) {
//         completion = new TimeInitStressorCompletion(durationMillis);
//      } else {
//         completion = new OperationCountInitCompletion(new AtomicInteger(numRequests));
//      }
...
//
//      for (InitStressor stressor : stressors) {
...
//   }
   ...
   protected void executeOperations() throws InterruptedException {
      ...
      for (int threadIndex = stressors.size(); threadIndex < numThreads; threadIndex++) {
         InitStressor stressor = new InitStressor(threadIndex, getLogic());
...
//   protected void setStressorCompletion(InitStressorCompletion completion) {
...
//      Object run(InitStressor stressor, int iteration);
   }
...
//      @Override
//      public Object run(InitStressor stressor, int iteration) {
...
//   }
   ...
   protected class InitStressor extends Thread {
      private int threadIndex;
      ...
      public InitStressor(int threadIndex, OperationLogic logic) {
         super("InitStressor-" + threadIndex);         
         this.threadIndex = threadIndex;
         this.logic = logic;
...
//   abstract class InitStressorCompletion {
// 
...
//   }
//
//   class OperationCountInitCompletion extends InitStressorCompletion {
//      
//      private final AtomicInteger requestsLeft;
//
//      OperationCountInitCompletion(AtomicInteger requestsLeft) {
...
//   }
//   
//   class TimeInitStressorCompletion extends InitStressorCompletion {
//      
...
//      
//      TimeInitStressorCompletion(long durationMillis) {
//         this.durationMillis = durationMillis;         
//      }
//
//      @Override
