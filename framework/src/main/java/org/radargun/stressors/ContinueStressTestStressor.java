...
public class ContinueStressTestStressor extends AbstractCacheWrapperStressor {

   private static final Log log = LogFactory.getLog(ContinueStressTestStressor.class);
   ...
   private volatile ContinueStressorCompletion completion;
   private volatile boolean finished = false;
   private volatile boolean terminated = false;
   private AtomicLong keysLoaded = new AtomicLong(0);
   
   protected List<ContinueStressor> stressors = new ArrayList<ContinueStressor>(numThreads);
   ...
   public Map<String, Object> stress(CacheWrapper wrapper) {
      init(wrapper);
      ContinueStressorCompletion completion;
      if (durationMillis > 0) {
         completion = new TimeContinueStressorCompletion(durationMillis);
      } else {
         completion = new OperationCountContinueCompletion(new AtomicInteger(numRequests));
      }
   ...
   protected Map<String, Object> processResults() {
      ...
      for (ContinueStressor stressor : stressors) {
   ...
   protected void executeOperations() throws InterruptedException {
      ...
      for (int threadIndex = stressors.size(); threadIndex < numThreads; threadIndex++) {
         ContinueStressor stressor = new ContinueStressor(threadIndex, getLogic());
   ...
   protected void finishOperations() {
      finished = true;
      synchronizer.masterPhaseEnd();
      for (ContinueStressor s : stressors) {
         try {
            s.join();
         } catch (InterruptedException e) {
            throw new RuntimeException(e);
   ...
   protected void setStressorCompletion(ContinueStressorCompletion completion) {
   ...
   protected interface OperationLogic {
      ...
      Object run(ContinueStressor stressor, int iteration);
   }
...
//               cacheWrapper.put(bucketId, key, value);
   ...
   protected class ContinueStressor extends Thread {
      private int threadIndex;
      ...
      public ContinueStressor(int threadIndex, OperationLogic logic) {
         super("ContinueStressor-" + threadIndex);         
         this.threadIndex = threadIndex;
         this.logic = logic;
   ...
   abstract class ContinueStressorCompletion {
   ...
   class OperationCountContinueCompletion extends ContinueStressorCompletion {
      
      private final AtomicInteger requestsLeft;

      OperationCountContinueCompletion(AtomicInteger requestsLeft) {
   ...
   class TimeContinueStressorCompletion extends ContinueStressorCompletion {
      ...
      TimeContinueStressorCompletion(long durationMillis) {
         this.durationMillis = durationMillis;         
      }
   ...
   public String toString() {
      return "ContinueStressTestStressor{" +
