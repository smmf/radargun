...
MASTER=10.1.2.6 # this was modified!!
...
export RADARGUN_DIR=/home/ubuntu/radargun/target/distribution/RadarGun-1.1.0-SNAPSHOT
export SEQUENCER_DIR=/home/ubuntu/radargun/my-stuff/zmq-sequencer
...
GOSSIP_ROUTER=1
#uncoment to start 0Mq sequencer (used in futuregrid)
ZMQ_SEQUENCER=1
...
BENC_GEN=${RADARGUN_DIR}/bin/benchmark-config-gen.sh
...
#else
cat ../machines | xargs -n1 -P 1024 -iserver rsync -r /home/ubuntu/radargun/target/distribution/RadarGun-1.1.0-SNAPSHOT/ server:/home/ubuntu/radargun/target/distribution/RadarGun-1.1.0-SNAPSHOT > /dev/null 2>&1
...
java -cp ${RADARGUN_DIR}/plugins/ff2-lf/lib/jgroups*.jar org.jgroups.stack.GossipRouter -port 13248 -bindaddress ${MASTER} > /dev/null &
}

start_zmq_sequencer() {
echo "start 0MQ sequencer in ${MASTER}"
#java -cp ${RADARGUN_DIR}/plugins/infinispan4/lib/jgroups*.jar org.jgroups.stack.GossipRouter -port 13248 -bindaddress ${MASTER} > /dev/null &
java -cp ${SEQUENCER_DIR}/jeromq-0.3.2-SNAPSHOT.jar:${SEQUENCER_DIR}/pubsub-bench-1.0-SNAPSHOT.jar bench.pubsub.ZeroMQSequencer & 
#java -cp ${SEQUENCER_DIR}/jzmq-3.0.1.jar:${SEQUENCER_DIR}/pubsub-bench-1.0-SNAPSHOT.jar bench.pubsub.ZeroMQSequencer & 
}
