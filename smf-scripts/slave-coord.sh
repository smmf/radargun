...
HOST_NAME=`hostname`
echo "[Coordinator] java -server -Xmx15G -Xms4G -XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode ${D_VARS} -classpath $CP org.radargun.Slave ${CONF}" > ${LOCAL_PREFIX}stdout_slave_${HOST_NAME}.out
echo "--------------------------------------------------------------------------------" >> ${LOCAL_PREFIX}stdout_slave_${HOST_NAME}.out
#nohup java -server -Xmx15G -Xms4G -XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode ${D_VARS} -classpath $CP org.radargun.Slave ${CONF} >> ${LOCAL_PREFIX}stdout_slave_${HOST_NAME}.out 2>&1 &
nohup java -verbose:gc -Xmx5000M -Xms3000M -server -XX:+UseConcMarkSweepGC -XX:+AggressiveOpts -verbose:gc -XX:ParallelGCThreads=4 -XX:NewRatio=5 -XX:SurvivorRatio=2 -Dhazelcast.operation.thread.count=4 ${D_VARS} -classpath $CP org.radargun.Slave ${CONF} >> ${LOCAL_PREFIX}stdout_slave_${HOST_NAME}.out 2>&1 &
