...
public class MyInfinispanWrapper implements CacheWrapper {
...
//            postSetUpInternal(confAttributes);
    ...
    private void setUpTransactionManager() {
        ...
        if (cache == null) {
            String msg = "Failed to get a cache to init the tx manager!!!!";
            log.error(msg);
        ...
        if (tm == null) {
            String msg = "REQUIRE a TX MANAGER!";
            log.error(msg);
            System.out.println(msg);
            System.exit(1);
        }
    ...
    protected String getConfigFile(TypedProperties confAttributes) {
        ...
        if (confAttributes.containsKey("config")) {
            return confAttributes.getProperty("config");
        }
        if (confAttributes.containsKey("file")) {
            return confAttributes.getProperty("file");
        }
        return config;
    }
...
//                    + Utils.prettyPrintMillis(gracePeriod));
//        }
//    }
...
//   }
    ...
    private void assertTm() {
        if (tm == null) {
            throw new RuntimeException("No configured TM!");
        }
    }
